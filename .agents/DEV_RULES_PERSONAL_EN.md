...
export function FoodList() {
  const foods = useLiveQuery(() =>
    db.entities.where("type").equals("food").toArray(),
  );
  return (
    <ul>
      {foods?.map((food) => (
...
_This guide provides a complete description of the Personal Life OS architecture. Following it, you will be able to build a scalable, offline-first application that combines finance, nutrition, and workouts._
